...
mod tests {
    ...
    fn honest_framed_codec_rejects_identity_and_bound_mutations() {
        let identity = honest_identity(RpcRole::Control, RpcMethod::KvPutDurable);
    ...
    fn test_rpc_method_from_u16_exhaustive() {
        ...
        assert_eq!(RpcMethod::from_u16(0x0240), None);
        let mut retired = encode_request(11, RpcMethod::KvPutDurable, &[]);
        retired[8..10].copy_from_slice(&0x0240_u16.to_le_bytes());
        assert!(decode_request(&retired).is_none());
        let identity = honest_identity(RpcRole::Control, RpcMethod::KvPutDurable);
        let mut framed = encode_honest_request(identity, &[]).unwrap();
        framed[32..34].copy_from_slice(&0x0240_u16.to_le_bytes());
        assert!(decode_honest_request(&framed).is_err());
