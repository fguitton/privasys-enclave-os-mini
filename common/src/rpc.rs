...
pub const RAFT_READY_BATCH_PROFILE_VERSION: u16 = 2;
...
const RAFT_READY_BATCH_HEADER_SIZE: usize = 44;
...
pub struct PersistRaftReadyBatch {
    ...
    /// Fresh enclave-generated namespace for one process-local protection
    /// key. It prevents a restarted adapter from colliding with ciphertext it
    /// cannot decrypt from an earlier process.
...
pub fn decode_persist_raft_ready_batch(
    ...
    let persistence_epoch = read_u64(18)?;
    let batch_id = read_u64(26)?;
    let expected_previous_durable_id = read_u64(34)?;
